...
async function validate(serviceId, service, servicesTermsTypes) {
  ...
  try {
    ...
    console.log(`rm declarations/${serviceId}.json`);
    return;
  }
        ...
        await Promise.all(terms.sourceDocuments.map(async sourceDocument => {
          ...
          try {
            ...
            if (err instanceof FetchDocumentError) {
              console.log(`rm declarations/${serviceId}.json # ${type} fetch ${location}`);
            } else if (err instanceof ExtractDocumentError) {
              console.log(`rm declarations/${serviceId}.json # ${type} extract ${location}`);
            } else {
              console.log(`rm declarations/${serviceId}.json # ${type} other ${location}`);
            }
          }
      }));
    }));
